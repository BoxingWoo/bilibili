...
#pragma mark 构造方法
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSArray<NSString *> *)otherButtonTitles onButtonTouchUpInside:(void (^)(BSAlertView * _Nonnull, NSInteger))onButtonTouchUpInsideBlock
{
    if (self = [super init]) {
        ...
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:cancelButtonTitle style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            if (onButtonTouchUpInsideBlock) {
                onButtonTouchUpInsideBlock(self, 0);
            }
        ...
        for (NSInteger i = 0; i < otherButtonTitles.count; i++) {
            UIAlertAction *alertAction = [UIAlertAction actionWithTitle:otherButtonTitles[i] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                if (onButtonTouchUpInsideBlock) {
                    onButtonTouchUpInsideBlock(self, i + 1);
                }
                [alertController dismissViewControllerAnimated:YES completion:NULL];
            }];
